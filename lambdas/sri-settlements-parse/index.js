...
async function procesarPDF(buffer, filename, s3Key = null, userId = null) {
  ...
  if (event.Records?.[0]?.eventSource === 'aws:s3') {
    ...
    for (const record of event.Records) {
      ...
      try {
        ...
        const userId = resp.Metadata && resp.Metadata.userid || null;
        const res    = await procesarPDF(buffer, filename, key, userId);
        resultados.push(res);
